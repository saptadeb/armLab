...
def blockDetector(hsvImg):
	...
	black_lo = np.array([0, 0, 20])
	black_hi = np.array([180, 255, 110])
	red_lo = np.array([170, 190, 130])
	red_hi = np.array([180, 230, 170])
	purple_lo = np.array([150, 80, 120])
	purple_hi = np.array([165, 130, 150])
	green_lo = np.array([50, 40, 115])
	green_hi = np.array([70, 75, 135])
	blue_lo = np.array([110, 90, 120])
	blue_hi = np.array([120, 255, 255])
	...
	for k in range(len(colorRangesLo)):
		...
		for i in range(len(contours)):
			...
			if(area < 300): # Filter too small ones
			   continue
			rect = cv2.minAreaRect(contour)
			(center_x, center_y) = rect[0]
			(width, height) = rect[1]
			coutour_orientation = rect[2]
			block_detections.append([int(center_x), int(center_y), width, height,area, coutour_orientation, k])
	...
	cv2.drawContours(rgbImg, allContours, -1, (0,255,0), 1)
	cv2.namedWindow("res_window",cv2.WINDOW_AUTOSIZE)
	cv2.imshow('res_window', rgbImg)
	cv2.waitKey()
